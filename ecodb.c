...
EcoDbRecord ecodb_data[] =
{
	...
	{ "D85", "Grünfeld, exchange variation, Nadanian Variation" },
...
int ecodb_cnt()
{
	return sizeof( ecodb_data ) / sizeof( EcoDbRecord );
}

const EcoDbRecord* ecodb_get( int index )
{
	dbgutil_test( index < ecodb_cnt() );

	return &( ecodb_data[ index ] );
}
